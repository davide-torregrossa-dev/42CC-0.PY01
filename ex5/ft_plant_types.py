...
{self._age_in_days} days old"
    ...
    def set_age(self, days: int):
        match self.age_is_valid(days):
            case 0:
                print(f"{self.icon} - {self.name}: Error,"
            ...
            case -1:
                print(f"{self.icon} - {self.name}: Error, "
                      f"Your plant can't last so many days!")
                print("🚫 Age update rejected")
                return
...
class Tree(Plant):
    ...
    def produce_shade(self):
        ...
        print(f"The {self.name} now produces a shade of"
...
if __name__ == "__main__":
    ...
    days_for_tomato = 20
    print(f"[making the {tomato.name} "
          f"grow and age for {days_for_tomato} days]")
